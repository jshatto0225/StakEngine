...
template <typename T> struct Mat<T, 4, 4> {
  ...
  static const u64 num_rows = 4;
  static const u64 num_cols = 4;
  static const u64 row_size = 4;
  ...
  template <typename U> Mat<T, 4, 4> &operator=(const Mat<U, 4, 4> &other);

  template <typename U> Mat<T, 4, 4> &operator+=(const Mat<U, 4, 4> &other);

  template <typename U> Mat<T, 4, 4> &operator+=(U val);
