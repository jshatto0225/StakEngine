...
template <typename T> template <typename U> inline Vec<T, 1>::Vec(U val) {
  x = static_cast<T>(val);
}

template <typename T> inline T &Vec<T, 1>::operator()(u32 i) {
  if (i != 0)
    throw std::invalid_argument("ERROR: Subsript out of range");
  return data[i];
}

template <typename T> inline T Vec<T, 1>::operator()(u32 i) const {
  if (i != 0)
    throw std::invalid_argument("ERROR: Subsript out of range");
  return data[i];
}
...
template <typename T> inline Vec<T, 1> &Vec<T, 1>::operator++() {
  ++x;
  return *this;
}
...
template <typename T> inline Vec<T, 1> Vec<T, 1>::operator++(i32) {
  Vec<T, 1> result(*this);
  ++*this;
  return result;
}
