...
template<typename T>
inline T& Vec<T, 4>::operator()(u32 i)
{
    if(i > 3 || i < 0)
...
template<typename T>
inline T Vec<T, 4>::operator()(u32 i) const
{
    if(i > 3 || i < 0)
        throw std::invalid_argument("ERROR: Subsript out of range");
    return data[i];
...
template<typename T>
inline Vec<T, 4>& Vec<T, 4>::operator++()
{
    ++x;
    ++y;
    ++z;
...
template<typename T>
inline Vec<T, 4> Vec<T, 4>::operator++(i32)
{
    Vec<T, 4> result(*this);
    ++*this;
    return result;
...
inline Vec<T, 4> operator+(const Vec<T, 4>& vec, U scalar)
{
    return Vec<T, 4>
    {
        vec.x + static_cast<T>(scalar),
        vec.y + static_cast<T>(scalar),
        vec.z + static_cast<T>(scalar),
        vec.w + static_cast<T>(scalar)
    };
}
...
inline Vec<T, 4> operator+(const Vec<T, 4>& vec1, const Vec<U, 4>& vec2)
{
    return Vec<T, 4>
    {
        vec1.x + static_cast<T>(vec2.x),
        vec1.y + static_cast<T>(vec2.y),
        vec1.z + static_cast<T>(vec2.z),
        vec1.w + static_cast<T>(vec2.w)
    };
}
...
inline Vec<T, 4> operator+(T scalar, const Vec<U, 4>& vec)
{
    return Vec<T, 4>
    {
        scalar + static_cast<T>(vec.x),
        scalar + static_cast<T>(vec.y),
        scalar + static_cast<T>(vec.z),
        scalar + static_cast<T>(vec.w)
    };
}
