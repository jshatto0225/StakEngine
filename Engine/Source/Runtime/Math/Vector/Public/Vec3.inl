...
template<typename T>
inline T& Vec<T, 3>::operator()(u32 i)
{
    if(i > 2 || i < 0)
...
template<typename T>
inline T Vec<T, 3>::operator()(u32 i) const
{
    if(i > 2 || i < 0)
        throw std::invalid_argument("ERROR: Subsript out of range");
    return data[i];
...
template<typename T>
inline Vec<T, 3>& Vec<T, 3>::operator++()
{
    ++x;
    ++y;
    ++z;
...
template<typename T>
inline Vec<T, 3> Vec<T, 3>::operator++(i32)
{
    Vec<T, 3> result(*this);
    ++*this;
    return result;
