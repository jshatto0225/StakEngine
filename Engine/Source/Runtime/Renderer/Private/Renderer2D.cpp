...
void Renderer2D::init() {
  // QUADS
  data.quad_vertex_array = PlatformManager::new_vertex_array();
  data.quad_vertex_buffer = PlatformManager::new_vertex_buffer(data.max_vertices * sizeof(QuadVertex));
  ...
  Shared<IndexBuffer> quad_index_buffer = PlatformManager::new_index_buffer(quad_indices, data.max_indices);
  data.quad_vertex_array->set_index_buffer(quad_index_buffer);
  delete[] quad_indices;

  // CIRCLES
  data.circle_vertex_array = PlatformManager::new_vertex_array();
  data.circle_vertex_buffer = PlatformManager::new_vertex_buffer(
      data.max_vertices * sizeof(CircleVertex));
  ...
  // LINES
  data.line_vertex_array = PlatformManager::new_vertex_array();
  data.line_vertex_buffer =
      PlatformManager::new_vertex_buffer(data.max_vertices * sizeof(LineVertex));
  ...
  // TEXT
  data.text_vertex_array = PlatformManager::new_vertex_array();
  data.text_vertex_buffer = PlatformManager::new_vertex_buffer(data.max_vertices * sizeof(TextVertex));
  ...
  data.white_texture = PlatformManager::new_texture_2d(TextureSpecification());
  ...
  data.quad_shader = PlatformManager::new_shader("Quad2DVertex.glsl", "Quad2DFragment.glsl");
  data.circle_shader = PlatformManager::new_shader("Circle2DVertex.glsl", "Circle2DFragment.glsl");
  data.line_shader = PlatformManager::new_shader("Line2DVertex.glsl", "Line2DFragment.glsl");
  data.text_shader = PlatformManager::new_shader("Text2DVertex.glsl", "Text2DFragment.glsl");
...
void Renderer2D::flush() {
  ...
  if (data.text_index_count) {
    ...
    // Draw quadIndexCount quads fron quadVertexArray
    RenderCommand::draw_indexed(data.text_vertex_array, data.text_index_count);
  }
}
