...
void sk_renderer_2d_init() {
  // QUADS
  data.quad_vertex_array = sk_create_vertex_array();
  data.quad_vertex_buffer = sk_create_vertex_buffer(data.max_vertices * sizeof(QuadVertex));
  ...
  IndexBuffer *quad_index_buffer = sk_create_index_buffer(quad_indices, data.max_indices);
  data.quad_vertex_array->set_index_buffer(quad_index_buffer);
  delete[] quad_indices;

  // CIRCLES
  data.circle_vertex_array = sk_create_vertex_array();
  data.circle_vertex_buffer = sk_create_vertex_buffer(data.max_vertices * sizeof(CircleVertex));
  ...
  // LINES
  data.line_vertex_array = sk_create_vertex_array();
  data.line_vertex_buffer = sk_create_vertex_buffer(data.max_vertices * sizeof(LineVertex));
  ...
  // TEXT
  data.text_vertex_array = sk_create_vertex_array();
  data.text_vertex_buffer = sk_create_vertex_buffer(data.max_vertices * sizeof(TextVertex));
  ...
  data.white_texture = sk_create_texture_2d(TextureSpecification());
  ...
  data.quad_shader = sk_create_shader("Quad2DVertex.glsl", "Quad2DFragment.glsl");
  data.circle_shader = sk_create_shader("Circle2DVertex.glsl", "Circle2DFragment.glsl");
  data.line_shader = sk_create_shader("Line2DVertex.glsl", "Line2DFragment.glsl");
  data.text_shader = sk_create_shader("Text2DVertex.glsl", "Text2DFragment.glsl");
...
void sk_renderer_2d_shutdown() {
  delete data.quad_shader;
  delete data.circle_shader;
  delete data.line_shader;
  delete data.text_shader;

  delete data.white_texture;

  delete data.quad_vertex_array;
  delete data.quad_vertex_buffer;

  delete data.circle_vertex_array;
  delete data.circle_vertex_buffer;

  delete data.line_vertex_array;
  delete data.line_vertex_buffer;

  delete data.text_vertex_array;
  delete data.text_vertex_buffer;
...
void sk_renderer_2d_flush() {
  if (data.quad_index_count) {
    ...
    sk_render_command_draw_indexed(data.quad_vertex_array, data.quad_index_count);
  }
  ...
  if (data.circle_index_count) {
    ...
    // Draw quadIndexCount quads fron quadVertexArray
    sk_render_command_draw_indexed(data.circle_vertex_array, data.circle_index_count);
  }
  ...
  if (data.line_vertex_count) {
    ...
    // Draw quadIndexCount quads fron quadVertexArray
    sk_render_command_set_line_width(data.line_width);
    sk_render_command_draw_lines(data.line_vertex_array, data.line_vertex_count);
  }
  ...
  if (data.text_index_count) {
    ...
    // Draw quadIndexCount quads fron quadVertexArray
    sk_render_command_draw_indexed(data.text_vertex_array, data.text_index_count);
  }
}
