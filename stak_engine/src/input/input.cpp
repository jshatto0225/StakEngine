...
f32 Input::get_mouse_x() {
    return mouse_x;
}

f32 Input::get_mouse_y() {
    return mouse_y;
}

Input_State Input::get_key(Key_Code key) {
    return keys[key];
}
    
Input_State Input::get_mouse_button(Mouse_Code button) {
    return mouse_buttons[button];
}

void Input::add_key_state_callback(Input_State state, Key_Code key, void(*func)()) {
    switch (state) {
...
void Input::add_generic_key_state_callback(Input_State state, void(*func)(Key_Code)) {
    switch (state) {
    case Input_State::DOWN:
...
void Input::add_generic_key_callback(void(*func)(Input_State, Key_Code)) {
    generic_key_callbacks.push_back(func);
}

void Input::add_mouse_button_state_callback(Input_State state, Mouse_Code button, void(*func)()) {
    switch (state) {
    case Input_State::DOWN:
...
void Input::add_generic_mouse_button_state_callback(Input_State state, void(*func)(Mouse_Code)) {
    switch (state) {
    case Input_State::DOWN:
...
void Input::add_generic_mouse_button_callback(void(*func)(Input_State, Mouse_Code)) {
    generic_mouse_button_callbacks.push_back(func);
}

void Input::add_mouse_move_callback(void(*func)(f32, f32)) {
...
void Input::set_raw_input(Platform::Window *window, bool value) {
    if (raw_input != value) {
        raw_input = value;
        if (value) {
            if (cursor_visibility == Cursor_Visibility::DISABLED) {
                Platform::enable_raw_input(window);
            }
            else {
            ...
            Platform::disable_raw_input(window);
        }
    }
}

void Input::set_cursor_visibility(Platform::Window *window, Cursor_Visibility visibility) {
    if (visibility != cursor_visibility) {
        ...
        if (visibility == Cursor_Visibility::DISABLED) {
            Platform::hide_cursor(window);
        } else {
            Platform::show_cursor(window);
        }
    }
}
