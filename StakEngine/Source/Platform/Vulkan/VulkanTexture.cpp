...
FVulkanTexture::FVulkanTexture(VkDevice Device, VkSwapchainKHR Swapchain, FUInt32 ImageCount, VkExtent2D Extent, VkFormat Format) : Device(Device), Format(Format), Extent(Extent), Backbuffer(true) {
    ...
    for (FUInt32 ImageViewIndex = 0; ImageViewIndex < Images.size(); ImageViewIndex++) {
        ImageViews[ImageViewIndex] = VulkanCreateImageView(Device, Images[ImageViewIndex], Format);
    }
